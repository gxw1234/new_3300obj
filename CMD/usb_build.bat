...
if exist test_stm32h750_cmake.elf (
    echo [Build] Found ELF file, generating binary and hex files...
    
    rem Generate BIN file
    arm-none-eabi-objcopy -O binary test_stm32h750_cmake.elf usb_app.bin
    echo [Build] Created: usb_app.bin
    
    rem Generate HEX file
    arm-none-eabi-objcopy -O ihex test_stm32h750_cmake.elf usb_app.hex
    echo [Build] Created: usb_app.hex
) else (
    echo [Build] ERROR: test_stm32h750_cmake.elf not found!
